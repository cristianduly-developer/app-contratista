...
export default async function handler(req, res) {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.error('[registrar-demo] Missing Supabase env vars')
  ...
  if (serviceKey) {
    ...
    validUntil.setDate(validUntil.getDate() + DEMO_DIAS)
    await supaAdmin.from('tenant_access').upsert({
      user_id: user.id,
  ...
  central.from('notificaciones_admin').insert({
    tipo: 'nueva_org',
    mensaje: `Nueva cuenta demo en App Contratista — ${nombre} (${email})`,
    org_id: orgId,
    app_id: APP_ID,
  }).then(() => {})
  ...
  try {
    ...
    const mailFrom = process.env.MAIL_FROM ?? 'App Contratista <noreply-contratista@solucionesmdp.com.ar>'
    const appUrl = 'https://contratista.solucionesmdp.com.ar'
    ...
    const bienvenidaHtml = `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
        <div style="background:#EA580C;padding:32px 24px;text-align:center;">
          ...
          <h1 style="color:white;margin:8px 0 4px;font-size:22px;">App Contratista</h1>
        ...
        <div style="padding:32px 24px;">
          ...
          <div style="background:#f9fafb;border-radius:10px;padding:20px;margin-bottom:24px;">
            ...
            <p style="margin:0 0 8px;color:#374151;font-size:14px;">✅ Presupuestar obras con la calculadora por m²</p>
            <p style="margin:0 0 8px;color:#374151;font-size:14px;">✅ Gestionar gremios, pagos y saldos</p>
            <p style="margin:0 0 8px;color:#374151;font-size:14px;">✅ Ver tu ganancia en tiempo real</p>
            <p style="margin:0 0 0;color:#374151;font-size:14px;">✅ Compartir links de avance con los gremios</p>
          </div>
          <div style="text-align:center;">
            <a href="${appUrl}" style="display:inline-block;background:#EA580C;color:white;padding:14px 32px;border-radius:10px;font-weight:700;font-size:15px;text-decoration:none;">Abrir App Contratista →</a>
          </div>
          <div style="margin-top:20px;padding:16px;background:#fff7ed;border-radius:10px;text-align:center;">
            <p style="margin:0 0 10px;font-size:14px;color:#374151;">¿No sabés por dónde empezar?</p>
            <a href="${appUrl}/ayuda" style="display:inline-block;background:#fff;color:#EA580C;padding:10px 24px;border-radius:8px;font-weight:600;font-size:13px;text-decoration:none;border:1px solid #fed7aa;">Ver guía de ayuda 📖</a>
          </div>
        </div>
        <div style="border-top:1px solid #f1f5f9;padding:20px 24px;text-align:center;">
          <p style="margin:0;color:#9ca3af;font-size:12px;">Soluciones MDP · <a href="https://wa.me/5492235767784" style="color:#9ca3af;">WhatsApp</a> · <a href="https://www.instagram.com/soluciones_mdp" style="color:#9ca3af;">Instagram</a> · <a href="https://www.facebook.com/share/1D7keoQJe1/" style="color:#9ca3af;">Facebook</a></p>
          <p style="margin:4px 0 0;color:#9ca3af;font-size:11px;">Seguinos en nuestras redes para enterarte de novedades y tips</p>
        </div>
      </div>`
    ...
    await Promise.all([
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: mailFrom,
          to: email,
          subject: `¡Bienvenido/a a App Contratista! Tu prueba de ${DEMO_DIAS} días está activa`,
          html: bienvenidaHtml,
        }),
      }),
      fetch('https://api.resend.com/emails', {
